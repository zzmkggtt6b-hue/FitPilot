...
export async function addTelegramUserMessage(userId: string, content: string, telegramUpdateId?: number): Promise<boolean> { const { error } = await supabaseAdmin.from("conversation_messages").insert({ user_id: userId, role: "user", content, telegram_update_id: telegramUpdateId ?? null }); if (!error) return true; if (error.code === "23505" && telegramUpdateId != null) return false; throw error; }
export async function addMessage(userId: string, role: "user" | "assistant", content: string) { const { error } = await supabaseAdmin.from("conversation_messages").insert({ user_id: userId, role, content }); if (error) throw error; }
