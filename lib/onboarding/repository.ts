...
export async function resetProfile(userId: string) {
  const results = await Promise.all([
    supabaseAdmin.from("fitness_goals").delete().eq("user_id", userId),
...
export async function setState(userId: string, state: OnboardingState, completed = false, pausedFromState?: OnboardingState | null) {
  const { data, error } = await supabaseAdmin.from("onboarding_sessions").update({
    current_state: state,
    completed,
    completed_at: completed ? new Date().toISOString() : null,
    paused_from_state: state === "PAUSED" ? (pausedFromState ?? null) : null,
  }).eq("user_id", userId).select("id,current_state").maybeSingle();
  if (error) throw error;
...
export async function addTelegramUserMessage(userId: string, content: string, telegramUpdateId?: number): Promise<string | false> {
  const { data, error } = await supabaseAdmin.from("conversation_messages")
    .insert({ user_id: userId, role: "user", content, telegram_update_id: telegramUpdateId ?? null, processing_status: "received" })
    .select("id")
    .single();
...
export async function markMessageProcessing(messageId: string) {
  const { error } = await supabaseAdmin.from("conversation_messages")
    .update({ processing_status: "processing", processing_error: null })
    .eq("id", messageId);
  if (error) throw error;
}

export async function markMessageCompleted(messageId: string) {
  const { error } = await supabaseAdmin.from("conversation_messages")
    .update({ processing_status: "completed", processing_error: null, processed_at: new Date().toISOString() })
    .eq("id", messageId);
  if (error) throw error;
}
...
export async function markMessageFailed(messageId: string, errorMessage: string) {
  const { error } = await supabaseAdmin.from("conversation_messages")
    .update({ processing_status: "failed", processing_error: errorMessage.slice(0, 2000), processed_at: new Date().toISOString() })
    .eq("id", messageId);
  if (error) throw error;
}
...
export async function addMessage(userId: string, role: "user" | "assistant" | "system", content: string) {
  const { error } = await supabaseAdmin.from("conversation_messages").insert({
    ...
    role,
    content,
    processing_status: role === "assistant" ? "completed" : "received",
    processed_at: role === "assistant" ? new Date().toISOString() : null,
  });
  if (error) throw error;
}
