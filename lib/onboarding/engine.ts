...
function deterministicFields(message: string): Partial<ProfileData> {
  ...
  if (/\b(male|man|mannelijk|m)\b/.test(value)) fields.sex = "male";
  else if (/\b(female|vrouw|vrouwelijk|woman|f)\b/.test(value)) fields.sex = "female";
...
export async function processOnboardingMessage(userId: string, message: string): Promise<string> {
  ...
  if (state === "LANGUAGE") { const language = parseLanguage(trimmed); if (!language) return "Kies alsjeblieft Nederlands of English. 🇳🇱 / 🇬🇧"; await saveProfile(userId, { language }); await setState(userId, "CONSENT"); return prompts.CONSENT[language]; }
  if (isLanguageChange(trimmed)) { const parsed = parseLanguage(trimmed); if (parsed) { currentLanguage = parsed; await saveProfile(userId, { language: currentLanguage }); profile = await loadProfile(userId); return missingPrompt(state, profile) || (currentLanguage === "en" ? "Got it. We’ll continue in English." : "Prima. We gaan verder in het Nederlands."); } }
  if (isProfileSummaryRequest(trimmed)) { const next = missingPrompt(state, profile); return summary(profile) + (state === "REVIEW" ? (currentLanguage === "en" ? "\n\nDoes this look correct? Reply 'yes' to confirm, or tell me what to change." : "\n\nKlopt dit? Antwoord 'ja' om te bevestigen, of vertel wat ik moet aanpassen.") : next ? `\n\n${next}` : ""); }
  if (state === "CONSENT") { const consent = /^(ja|yes|y|akkoord|agree|i agree|ik ga akkoord)$/i.test(trimmed); if (!consent) return currentLanguage === "en" ? "No problem. Without consent I can’t save a personal fitness profile. Send 'yes' to continue, or tell me if you want to switch language." : "Geen probleem. Zonder akkoord kan ik geen persoonlijk fitnessprofiel opslaan. Stuur 'ja' om door te gaan, of zeg het als je van taal wilt wisselen."; await saveProfile(userId, { consent: true }); await setState(userId, "BASIC_PROFILE"); profile = await loadProfile(userId); return missingPrompt("BASIC_PROFILE", profile); }
  ...
  let extraction;
  try { extraction = await extractOnboarding({ state, message, profile }); }
  catch (error) { console.error("FitPilot extraction failed; using deterministic fallback", { error, userId, state }); extraction = { intent: "answer" as const, fields: deterministicFields(message) }; }
  const deterministic = deterministicFields(message);
  const fields = { ...deterministic, ...extraction.fields };
  if (extraction.intent === "language_change" || fields.language) { const language = requestedLanguage(String(fields.language ?? ""), currentLanguage); await saveProfile(userId, { language }); profile = await loadProfile(userId); return missingPrompt(state, profile) || (language === "en" ? "Got it. We’ll continue in English." : "Prima. We gaan verder in het Nederlands."); }
  if (extraction.intent === "restart") { await resetProfile(userId); await setState(userId, "LANGUAGE"); return prompts.LANGUAGE["nl"]; }
  if (extraction.intent === "question") { const next = missingPrompt(state, profile); return `${currentLanguage === "en" ? "Good question. Here’s what I currently have:" : "Goede vraag. Dit is wat ik momenteel van je heb:"}\n\n${summary(profile)}${next ? `\n\n${next}` : ""}`; }
  if (Object.keys(fields).length === 0) { const next = missingPrompt(state, profile); return currentLanguage === "en" ? `I couldn't extract any new profile information.${next ? ` ${next}` : " Please tell me a little more."}` : `Ik kon nog geen nieuwe profielinformatie vinden.${next ? ` ${next}` : " Kun je iets meer vertellen?"}`; }
  const { language: _language, ...profileFields } = fields; await saveProfile(userId, profileFields as ProfileData); profile = await loadProfile(userId); const newState = nextState(state, profile); await setState(userId, newState);
